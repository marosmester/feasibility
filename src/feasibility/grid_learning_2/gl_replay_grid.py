"""GL viewer for a grid_learning_2 dataset_grid2_*.h5 (generate_dataset.py's output): steps
through the sampled trials of one row, showing each lattice cell's ostrich/hstack FINAL poses on
the real terrain, for manual sanity-checking before the dataset is fed to a network -- does the
spawn lattice line up with the obstacle(s), are masked cells the ones actually on/near an obstacle
rather than an unexplained diverged solve, do a cell's two final poses look sane (not NaN, not
flung across the map).

Unlike replay/gl_replay.py there is no trajectory to interpolate: the file stores only
`y[R, G, G, 14]` (ostrich pose(7) + hstack pose(7) per lattice cell of the R-th row) and
`mask[R, G, G]` -- one keyframe per cell, not a time series. So this is a stepper, not a continuous
replay: it steps through a row's G x G cells (or one, via --cell), freezing the two robot meshes at
that cell's final poses each time, with no wheel spin (the dataset logs no wheel velocity to
integrate, only the final chassis pose).

**What differs from grid_learning's viewer, and why this is a separate script.** A v1 row has ONE
commanded yaw rate for all 225 cells, so that script reads `float(f["wz"][row])` once and prints it
in the header. A v2 row carries a command FIELD: `wz[row]` is `[G, G]`, one draw per cell, so the
command belongs on the per-cell status line, not the header -- cell (3,4) and cell (3,5) are
different commands on the same terrain, and that is exactly what this viewer now lets you see side
by side. Three reads change (`rows_per_map` rather than the dropped `n_commands` root attr, the
`wz` field, the status line); everything else -- row ordering, the spawn_xy/y/mask layout,
terrain_from_h5, the footprint filter, the SE(3) formula, all the rendering -- is v1's, verbatim.
Note the terrain mesh comes from the file's EMBEDDED RAW asset grid, not from `grid/heightmap`, so
v2's 81 x 81 @ 0.125 m CNN grid is invisible here.

The whole lattice is drawn as small static spheres (Newton ModelBuilder.add_shape_sphere, body=-1,
the same static-world pattern the ground mesh uses), three-way colored: green = mask=True (valid
solve), gray = never simulated (spawn footprint on an obstacle, recomputed locally via
footprint_clear()), red = was footprint-clear but mask is False anyway -- a diverged solve, the
case actually worth flagging. The currently-shown cell's sphere is recolored white as a highlight.

Deliberately independent of feasibility.grid_learning and feasibility.learning (design.md section
11): the footprint filter and the pose_error-style SE(3) formula are reimplemented here rather than
imported, so OBSTACLE_MARGIN_FRACTION below has to stay in step with generate_dataset.py's by hand.
feasibility.comparator and feasibility.heightmap are shared infrastructure and ARE imported.

There is no --nn-checkpoint flag yet: v2's train.py does not exist, so there is no checkpoint format
to load. When it lands, this is where it plugs in -- one `GridDivergenceNet.predict(heightmap, wz)`
call for the row's whole lattice (no spawn_xy, no extent: the readout is fixed integer geometry),
printing the predicted (e_pos, e_rot) next to the real one on each status line.

CLI parameters:
    --file PATH             dataset_grid2_*.h5 path (required -- filenames encode M/L/g, no
                            single sensible default)
    --map-index INT         which of the file's n_maps maps to show (default: 0)
    --row-index INT         which of that map's rows_per_map command fields (default: 0). v1 calls
                            this --command-index; here a row is a FIELD of commands, not one.
    --cell I J              freeze on one specific lattice cell instead of stepping through all of
                            them (skips --dwell/--loop)
    ...
    --dry-run               load + validate the row and print summary stats, no GL viewer -- the
                            only piece testable without a display, mirroring generate_dataset.py's
                            own +dry_run convention. Also lists the row's largest-divergence cells
                            with their own commands, as ready-to-paste --cell arguments.

Usage:
    python src/feasibility/grid_learning_2/gl_replay_grid.py --file outputs/dataset_grid2_large_box_random0_M2_L10_g15.h5 --dry-run
    python src/feasibility/grid_learning_2/gl_replay_grid.py --file outputs/dataset_grid2_large_box_random0_M2_L10_g15.h5
    python src/feasibility/grid_learning_2/gl_replay_grid.py --file outputs/dataset_grid2_large_box_random0_M2_L10_g15.h5 --map-index 1 --row-index 2
    python src/feasibility/grid_learning_2/gl_replay_grid.py --file outputs/dataset_grid2_large_box_random0_M2_L10_g15.h5 --cell 7 7 --which ostrich
    python src/feasibility/grid_learning_2/gl_replay_grid.py --file outputs/dataset_grid2_large_box_random0_M2_L10_g15.h5 --show-masked --loop --dwell 0.5
"""
...
# same convention plotting/terrain_visualizer.py uses for a static "look at the whole map" camera.
# The obstacle position is random per map, so there is no single obstacle_x to frame against.
CAMERA_YAW = 90.0
CAMERA_PITCH = -25.0
...
# Reference/spawn robot: every lattice cell's rollout starts from the SAME (yaw, resting-on-ground)
# configuration -- only its (x, y) differs per cell, and generate_dataset.py never logs that
# starting pose itself (only the two sims' FINAL poses). Parking one static, distinctly colored
# robot at a map corner -- outside the spawn lattice's own footprint, so it never overlaps a real
# trial -- gives a fixed "this is what upright, facing +X looked like before any of this diverged"
# reference alongside the moving ostrich/hstack pair.
...
# cells get simulated at all -- copied rather than imported (see module docstring), so this constant
# must stay in step with that module's own OBSTACLE_MARGIN_FRACTION by hand.
OBSTACLE_MARGIN_FRACTION = 0.15

N_WORST_CELLS = 5  # how many high-divergence cells --dry-run lists as --cell suggestions
...
def footprint_clear(terrain: HeightMapReader, xy: np.ndarray, spawn_yaw: float) -> np.ndarray:
    ...
    Note it takes no command: the blocked set is a property of the MAP alone, which is why it is
    the same for every row even though every row now carries different commands."""
...
def pose_to_se3(pose: np.ndarray) -> np.ndarray:
    """(x, y, z, qx, qy, qz, qw) -> 4x4 SE(3) matrix. Scalar counterpart of custom_dataset.py's
    vectorized poses_to_se3 -- this script shows one cell at a time, so the loop-free form buys
    nothing and the explicit matrix is easier to read against learning/pose_error.py."""
...
def se3_error(T1: np.ndarray, T2: np.ndarray) -> tuple[float, float]:
    """T_err = T1^-1 @ T2 -> (pos_error [m], rot_error [rad]) -- the same convention
    custom_dataset.py's se3_errors uses, so a number printed here is the label the network is
    trained against, not an approximation of it."""
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--file", type=pathlib.Path, required=True, help="dataset_grid2_*.h5 path")
    ...
    ap.add_argument("--row-index", type=int, default=0,
                    help="which of that map's command fields (default: 0)")
...
def main() -> None:
    ...
    with h5py.File(args.file, "r") as f:
        # `n_rows_per_map` is the same attr under its first name, kept readable so files generated
        # before the rename don't need regenerating; `n_rows` there and here is the TOTAL.
        rows_attr = next((k for k in ("rows_per_map", "n_rows_per_map") if k in f.attrs), None)
        if rows_attr is None:
            raise SystemExit(
                f"{args.file} has no `rows_per_map` root attr. A grid_learning (v1) file stores "
                "`n_commands` instead and one SCALAR wz per row -- view it with "
                "src/feasibility/grid_learning/gl_replay_grid.py, which this script deliberately "
                "does not replace."
            )
        n_maps, rows_per_map = int(f.attrs["n_maps"]), int(f.attrs[rows_attr])
        G = int(f.attrs["grid_n"])
        if not (0 <= args.map_index < n_maps):
            raise SystemExit(f"--map-index must be in [0, {n_maps}), got {args.map_index}")
        if not (0 <= args.row_index < rows_per_map):
            raise SystemExit(f"--row-index must be in [0, {rows_per_map}), got {args.row_index}")
        row = args.map_index * rows_per_map + args.row_index  # rows are map-major, as written

        spawn_yaw = float(f.attrs["spawn_yaw"])
        wz_field = f["wz"][row]  # [G, G] -- one command per cell, v1's is a single float
        if wz_field.ndim != 2:
            raise SystemExit(
                f"{args.file} stores wz with shape {f['wz'].shape}; this viewer needs a per-cell "
                f"command FIELD [R, {G}, {G}] (design.md section 7a)."
            )
    ...
    print(
        f"[row {row}]  map={map_path}  (map {args.map_index}, field {args.row_index})   "
        f"{G}x{G} lattice: {n_valid} valid, {n_blocked} blocked (obstacle), {n_diverged} diverged"
    )
    # The command is a field, so it gets its own line -- there is no single number for the row.
    # Reported over the VALID cells only: masked cells carry a real draw too (generate_dataset.py
    # samples them so the array is never structurally zero-filled), but nothing ever simulated it.
    wz_valid = wz_field[mask_row]
    if wz_valid.size:
        print(
            f"[command]  wz over valid cells: min {wz_valid.min():+.3f}, max {wz_valid.max():+.3f}, "
            f"mean {wz_valid.mean():+.3f} rad/s, {int((wz_valid == 0.0).sum())} at exactly 0 "
            f"-- one draw per cell, shown per cell below"
        )

    if args.dry_run:
        # The label the network actually fits, recomputed here from the stored poses. Listing the
        # worst cells with their own commands is the point of the exercise: a divergence field is
        # only trustworthy if its extremes are real collisions rather than solver noise, and each
        # line below is a ready-to-paste command for looking at one.
        errors = np.zeros((G, G, 2), dtype=np.float64)
        for i in range(G):
            for j in range(G):
                if mask_row[i, j]:
                    errors[i, j] = se3_error(pose_to_se3(y_row[i, j, :7]), pose_to_se3(y_row[i, j, 7:14]))
        e_pos = errors[..., 0][mask_row]
        e_rot = errors[..., 1][mask_row]
        if e_pos.size:
            print(
                f"[labels]   e_pos: median {np.median(e_pos):.3f}, q90 {np.quantile(e_pos, 0.9):.3f}, "
                f"max {e_pos.max():.3f} m   |   e_rot: median {np.median(e_rot):.3f}, "
                f"q90 {np.quantile(e_rot, 0.9):.3f}, max {e_rot.max():.3f} rad"
            )
            flat = errors[..., 0].copy()
            flat[~mask_row] = -1.0
            worst = np.dstack(np.unravel_index(np.argsort(flat, axis=None)[::-1], flat.shape))[0]
            print(f"[worst]    top {N_WORST_CELLS} cells by e_pos -- eyeball a few before trusting the file:")
            for i, j in worst[:N_WORST_CELLS]:
                print(
                    f"  e_pos={errors[i, j, 0]:.3f} m  e_rot={errors[i, j, 1]:.3f} rad  "
                    f"wz={wz_field[i, j]:+.3f} rad/s   "
                    f"--file {args.file} --map-index {args.map_index} "
                    f"--row-index {args.row_index} --cell {i} {j}"
                )
    ...
    def show_cell(i: int, j: int) -> None:
        ...
        # wz is this CELL's own command, not the row's -- the one line that could not carry over
        # from v1 unchanged, and the reason two adjacent spheres can disagree wildly.
        print(f"  [cell {i:2d},{j:2d}] xy=({x:+.2f},{y:+.2f})  wz={wz_field[i, j]:+.3f} rad/s  {status}")
    ...
    print(
        ...
        f"(grey robot at the corner = shared spawn config; each cell prints its OWN commanded omega_z)"
    )
