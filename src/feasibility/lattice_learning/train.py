...
* **The mirror augmentation is the scalar form**: `patch -> flip(patch, rows)`, `kappa -> -kappa`,
  labels unchanged (design.md section 7c). The patch's rows are body +Y (`PatchSpec.ys()`, asserted
  mirror-symmetric in model.py), so a row flip is exactly a reflection in the body XZ plane.
...
def top_decile_rows(target: torch.Tensor, decile: float = 0.1) -> torch.Tensor:
    ...
    e_pos = target[:, TARGET_NAMES.index("e_pos")]
...
def mirror_augment(
    ...
    """Per-sample, p=0.5: reflect the body-frame patch across y=0 and negate the curvature --
    an EXACT symmetry of the robot and the arc (design.md section 7c), so a free 2x on data.
        ...
        y     -> y                         e_pos, e_rot are magnitudes
    ...
    kappa = torch.where(flip, -kappa, kappa)
    return patch, kappa, y
...
def build_checkpoint(
    ...
    return dict(
        ...
        v_nom=float(ds.attrs.get("v_nom", V_NOM)),
...
def evaluate(
    ...
    loss_sum, n = 0.0, 0
    preds, targets = [], []
    for (patch, kappa), y in loader:
        patch, kappa, y = patch.to(device), kappa.to(device), y.to(device)
        y_hat = model(prepare_patch(patch, blur_terrain), kappa)
...
def mirror_equivariance_check(
    ...
    """max |f(patch, kappa) - f(flip(patch), -kappa)| in MODEL space over a few val rows. Not an
    assertion: a conv stack is not reflection-equivariant at init, so this is a training
    ...
    kappa = ds.kappa[idx].to(device)
    model.eval()
    a = model(patch, kappa)
    b = model(torch.flip(patch, dims=[-2]), -kappa)
...
def final_report(
    ...
    for subset_name, rows in subsets.items():
        ...
        for name, p in table.items():
            ...
            print(f"                 {name:<22s} {fmt(value)}")
            key = f"final_rmse_{subset_name}_{name}".replace(" ", "_")
            summary.update({f"{key}_{n}": v for n, v in zip(TARGET_NAMES, value.tolist())})
            if name == "model":
                model_rmse[subset_name] = value

    r2 = r_squared(pred, target)
    print(f"[final report] val R^2 per head (model):      {fmt(r2)}")
    summary.update({f"final_r2_{n}": v for n, v in zip(TARGET_NAMES, r2.tolist())})
    ...
    if bool(flat.any()):
        ...
        print(f"[final report] kappa~0 on flat ({int(flat.sum())} rows), predicted vs 0: {fmt(pred_zero)}")
        print(f"[final report] kappa~0 on flat ({int(flat.sum())} rows), TRUE vs 0:      {fmt(true_zero)}")
        summary.update({f"final_flat_zero_pred_{n}": v for n, v in zip(TARGET_NAMES, pred_zero.tolist())})
    else:
...
def train(args: argparse.Namespace, run: wandb.sdk.wandb_run.Run) -> pathlib.Path | None:
    ...
    for name, value in baseline_rmse.items():
        print(f"[baseline] {name:<22s} RMSE: {fmt(value)}")
        run.summary.update(
            {f"baseline_{name.replace(' ', '_')}_{n}": v for n, v in zip(TARGET_NAMES, value.tolist())}
        )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for (patch, kappa), y in train_loader:
            patch, kappa, y = patch.to(device), kappa.to(device), y.to(device)
            if not args.no_augment:
                patch, kappa, y = mirror_augment(patch, kappa, y)

            optimizer.zero_grad()
            y_hat = model(prepare_patch(patch, args.blur_terrain), kappa)
...
def self_test(args: argparse.Namespace) -> None:
    ...
    print("[augment] row flip + kappa negate, labels unchanged, involution ok")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for drop in (False, True):
            ...
            args.checkpoint = pathlib.Path(tmp) / f"checkpoint_drop{int(drop)}.pt"
            ...
            ds = ArcDivergenceDataset(path, drop_blocked_endpoints=drop)
            n_rows[drop] = len(ds)
            if drop:
            ...
            prediction = model.predict(ds.patch[:4], ds.kappa[:4])
            assert prediction.shape == (4, K) and torch.isfinite(prediction).all()
