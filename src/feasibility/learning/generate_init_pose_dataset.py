...
(sample_dataset()/legal_spawn_poses()/continuous_spawn_poses()) and the chunked ostrich/hstack
batch rollout (simulate_dataset_rollout()) this module is built on -- this file only decides what
gets written to the output HDF5's `per_variant`/`root` fields, everything else is shared with the
sibling generator, generate_dataset_body_centered_patch.py (same spawn sampling and simulator
rollout, but writing a body-frame terrain patch instead of the raw spawn pose as the model's
spatial feature -- see terrain_patch.py).
...
CLI parameters: see generate_dataset_utils.py's module docstring (+n_samples=, +seed=,
+box_height=, +duration_s=, +chunk=, +settle_steps=, +spawn_mode=, +mu=, +k_turn=, +device=, plus
any standard Hydra config-group override).
...
Usage:
    ...
    python src/feasibility/learning/generate_init_pose_dataset.py +box_height=0.5 +chunk=64
